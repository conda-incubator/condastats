...
@pytest.mark.parametrize("func_name,column,expected_value", [
...
@pytest.mark.parametrize("func_name", ["pkg_platform", "data_source", "pkg_version", "pkg_python"])
...
def test_monthly_aggregation(get_groupby, func_name):
    ...
    result = get_groupby(func_name, 'pandas', start_month=TEST_START_MONTH, end_month=TEST_END_MONTH, monthly=True)
